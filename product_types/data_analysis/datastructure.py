...
class Datastructure:
    ...
    def _build_self_link(self) -> dict:
        ...
        self_link["href"] = f"/mdr/{self.parent_product.product_type if self.parent_product.product_type.startswith('integrated/') else self.parent_product.model_type}/{version}{f'/{self.parent_product.product_subtype}' if  self.parent_product.product_subtype else ''}/datastructures/{name}"

...
class Varset:
    ...
    def __init__(self, varset_data, parent_product, parent_datastructure = None):
        ...
        self.name = self.transformer.cleanup_html_encoding(varset_data.get("name")).replace("Variables", "").strip()
    ...
    def _build_self_link(self) -> dict:
        ...
        self_link["href"] = f"/mdr/{self.parent_product.product_type if self.parent_product.product_type.startswith('integrated/') else self.parent_product.model_type}/{version}{f'/{self.parent_product.product_subtype}' if  self.parent_product.product_subtype else ''}/datastructures/{datastructure_name}/varsets/{name}"

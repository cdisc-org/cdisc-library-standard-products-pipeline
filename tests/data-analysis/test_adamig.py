...
@pytest.mark.parametrize("product_type", [("adamig"), ("adam-occds")])
def test_get_datastructures(
    ...
    config = Config({constants.DATASTRUCTURES: "12345"})
    version = "5-0"
    adamig = ADAMIG(
        mock_wiki_client,
        mock_library_client,
    ...
    datastructure_names = [
        c["fields"]["name"] for c in mock_datastructure_data["list"]["entry"]
    ]
    version_string = product_type + "-" + version
    for datastructure in datastructures:
        assert datastructure.name in datastructure_names
        assert (
            datastructure.links["self"]["href"]
            == f"/mdr/adam/{version_string}/datastructures/{datastructure.name.replace(' ', '')}"
        )
        assert (
            datastructure.links.get("parentProduct")
            == mock_adamig_summary["_links"]["self"]
        )


@pytest.mark.parametrize("product_type", [("adamig"), ("adam-occds")])
def test_get_varsets(
    mock_wiki_client,
    mock_library_client,
    mock_adamig_summary,
    mock_varset_data,
    ...
    for varset in varsets:
        ...
        assert (
            varset.links["self"]["href"]
            == f"/mdr/adam/{version_string}/datastructures/{parent_datastructure}/varsets/{varset.name.replace(' ', '')}"
        )
        assert (
            varset.links.get("parentProduct") == mock_adamig_summary["_links"]["self"]
        )


@pytest.mark.parametrize("product_type", [("adamig"), ("adam-occds")])
def test_get_all_prior_versions(
    mock_wiki_client, mock_library_client, mock_adamig_summary, product_type
):
    config = Config({})
    version = "5-0"
    adamig = ADAMIG(

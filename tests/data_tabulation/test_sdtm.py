...
from tests.conftest import mock_library_client, mock_wiki_client, mock_classes_data, mock_products_payload, mock_sdtm_summary
...
def test_get_classes(mock_wiki_client, mock_library_client, mock_sdtm_summary, mock_classes_data):
    config = Config({
...
def test_get_datasets(mock_wiki_client, mock_library_client, mock_sdtm_summary, mock_dataset_data):
    config = Config({
    ...
    for dataset in datasets:
        ...
        assert dataset.links["self"]["href"] == f"/mdr/{product_type}/{version}/datasets/{dataset.name.replace(' ', '')}"
...
def test_get_all_prior_versions(mock_wiki_client, mock_library_client, mock_sdtm_summary, mock_products_payload, product_type):
    config = Config({})
    version = "5-0"
    sdtm = SDTM(mock_wiki_client, mock_library_client,mock_sdtm_summary,product_type,version, config)
...
def test_get_described_value_domain(mock_wiki_client, mock_library_client, mock_sdtm_summary, mock_products_payload, 
                        described_value_domain, expected_output):
    config = Config({})
    version = "5-0"
    sdtm = SDTM(mock_wiki_client, mock_library_client,mock_sdtm_summary,"sdtm",version, config)
